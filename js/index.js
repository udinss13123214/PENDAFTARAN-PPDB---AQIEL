...
window.onload = function () {
    const tanggalPenutupan = localStorage.getItem('tanggalPenutupan');
    if (tanggalPenutupan) {
        const now = new Date();
        const closingDate = new Date(tanggalPenutupan);

        // Jika tanggal penutupan sudah lewat, sembunyikan tombol pendaftaran
        if (now > closingDate) {
            const daftarButtonSMA = document.getElementById("daftarbtnsma");
            const daftarButtonSMK = document.getElementById("daftarbtnsmk");

            if (daftarButtonSMA) {
                daftarButtonSMA.style.display = "none";
            }
            if (daftarButtonSMK) {
                daftarButtonSMK.style.display = "none";
            }
    ...
    // Tambahkan event listener untuk tombol pendaftaran
    const daftarButtonSMA = document.getElementById("daftarbtnsma");
    const daftarButtonSMK = document.getElementById("daftarbtnsmk");

    if (daftarButtonSMA) {
        daftarButtonSMA.addEventListener("click", () => checkLoginAndOpenModal("SMA"));
    }
    if (daftarButtonSMK) {
        daftarButtonSMK.addEventListener("click", () => checkLoginAndOpenModal("SMK"));
    }
};

// Fungsi pengecekan login dan membuka modal
function checkLoginAndOpenModal(type) {
    const nis = localStorage.getItem("nis");
    if (!nis) {
    ...
    modalContent.innerHTML = `
        <div class="p-6 w-4/5 max-w-2xl mx-auto bg-white rounded-lg shadow-lg">
            ...
            <div class="flex gap-2 mb-4">
                <input type="text" id="searchSchool" placeholder="Cari sekolah..." class="w-full p-2 border rounded" value="${searchQuery}">
...
// Fungsi pencarian sekolah (seperti sebelumnya)
function searchSchools(type) {
    const searchQuery = document.getElementById("searchSchool").value.toLowerCase();
    const schools = {
        "SMA": [
            { name: "SMA Negeri 2 Surakarta", description: "Sekolah unggulan dengan program akademik dan ekstrakurikuler yang kuat." },
            { name: "SMA Negeri 3 Surakarta", description: "Sekolah berbasis kurikulum nasional dengan fokus pada pengembangan karakter." },
            { name: "SMA Negeri 4 Surakarta", description: "Sekolah favorit dengan banyak prestasi akademik dan non-akademik." }
        ],
        "SMK": [
            { name: "SMK Negeri 4 Surakarta", description: "Menawarkan program keahlian di bidang bisnis dan teknologi." },
            { name: "SMK Negeri 6 Surakarta", description: "Sekolah kejuruan dengan fokus pada akuntansi dan teknologi informasi." },
            { name: "SMK Negeri 5 Surakarta", description: "Menghasilkan lulusan siap kerja dengan program keahlian unggulan." },
            { name: "SMK Negeri 7 Surakarta", description: "Sekolah dengan fasilitas lengkap dan tenaga pengajar profesional." }
        ]
    };

    const filteredSchools = schools[type].filter(school =>
        school.name.toLowerCase().includes(searchQuery) || school.description.toLowerCase().includes(searchQuery)
    );

    const schoolList = document.getElementById("schoolList");
    schoolList.innerHTML = filteredSchools.map(school => `
...
function submitSelection() {
    ...
    const role = localStorage.getItem("role");
    const nilaiRata = localStorage.getItem("nilaiRata") || "0";
    ...
    selectedChoices.forEach(choice => {
        ...
        const data = {
            name: name,
            nilaiRata: nilaiRata,
            status: "Menunggu" // Tambahan status default
        };
    ...
    if (role === "admin") {
        alert("Data berhasil terkirim (simulasi admin).");
    } else {
        if (!hasError) {
